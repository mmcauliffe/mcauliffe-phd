...
ggplot(plotData,aes(x=Step,y=ACC, colour=Experiment, group = Experiment)) + geom_point(size=1.7)+facet_grid(Attention~ExposureType, labeller=if_labeller) +geom_line() + geom_errorbar(aes(ymin=ACC-ci,ymax=ACC+ci),linetype='solid',size=0.1)+ ylab('Proportion /s/ response') +xlab('Continua step') + scale_x_continuous(breaks = 1:6)  + theme_bw() + theme(text=element_text(size=16))+scale_shape_manual(values = c(21, 22,23),labels = c('Experiment 1','Experiment 2','Control'))+scale_colour_manual(values = c("#0072B2", "#D55E00","#000000"),labels = c('Experiment 1','Experiment 2','Control'))

ggsave('../thesis/graphs/exp12_categresults.pdf',width=170,height=80,units='mm',dpi=600)
