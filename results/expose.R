...
experiment.2.expose.mod <- glmer(ACC ~ cTrial*itemtype2*Attention*ExposureType +(1|Subject) + (0+cTrial*itemtype2|Subject) + (1+Attention|Word), family='binomial',data=subset(expose.word,Experiment=='exp1'), control=glmerControl(optCtrl=list(maxfun=200000) ))
